...
class Binary:public Data{/*Converts real-world data into a type that is very similar to the binary number system, but with some differences, so that the artificial neural network can handle it.
                          ...
                          And the second the bits may be different from 0-1 values. For example, if an instance of this class is used in the output layer and the preferred activation function is TanH, the bits will be -1 or 1.
...
class TransformedData{/* Allows ANN data types(Binary, Flag, Narrowed and Native) to be managed through a single class.
                        ...
                        A real-world data is converted to either ANN data types for a neural network to handle. But regardless of Binary, Flag, Narrowed or Native types, a neural network will process them as TransformedData.
                      */
public:

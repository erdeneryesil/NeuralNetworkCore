...
Training an artificial neural network with all training data for one cycle is called 'epoch'. Class LogEW stores both total error values calculated and number of updated weights in a file at the end of an epoch. And repeats that every epoch. This file also contains the features that the artificial neural network had at the time of that training.
...
## Data Types
The real-world input and output data passed to an artificial neural network must be numerical in order to be handled by the artificial neural network. For this reason within the NeuralNetworkCore library, there are classes that convert real-world input and output values into data types that the artificial neural network can handle. These are Binary, Flag, Narrowed and Native classes. These classes will be called to as ‘ANN data types’.

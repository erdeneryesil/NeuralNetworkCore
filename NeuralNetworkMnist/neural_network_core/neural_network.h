...
class InputLayerPattern{/*Contains and manages the features of the input layer of a artificial neural network.
                          ...
                          But cells(or neurons) that construct the input layer of a artificial neural network are not included in this class.
...
public:
  ...
  TransformedData** createSample(const int*const originals)const;// Takes the real world data of a sample to be passed as input to the artificial neural network as parameters and returns them by converting to ANN data types. Parameters : Real world data of an example(originals(array)).
};
//-------------------------------------------------
class OutputLayerPattern:public InputLayerPattern{/*Contains and manages the features of the output layer of a artificial neural network.
                                                    ...
                                                    Cells(or neurons) that construct the output layer of a artificial neural network are not included in this class.
...
class HiddenLayerPattern{/* Contains and manages the features of the hidden layers of a artificial neural network.
                            ...
                            Cells(or neurons) that construct the hidden layers of a artificial neural network are not included in this class.
...
class NeuralNetwork{/*This class represents an artificial neural network.
                    ...
                    */
public:
    enum class ActionMode{Idle,Train,Test,Run};/*For GUI. Action modes that the artificial neural network has.
    						Idle : The artificial neural network is doing nothing. Waiting to be trained, tested or run.
    						Train : The artificial neural network is being trained.
    						Test : The artificial neural network is being tested.
    						Run : Inputs are passed to the artificial neural network and outputs are being calculated.
    						*/ 
    						
    enum class StatusTraining{Ongoing,Halted,Succesfull,Failed};/*For GUI. The current status of the training of the artificial neural network.
    								Ongoing : The training is ongoing.
    								Halted : The training has been halted by user.
    								Succesfull : The training has completed successfully.
    								Failed : The training has been halted due to incorrect results.
    								*/
    								
    enum class VerificationSamplesFile{None,Verified,Denied};/*For GUI. Verification status of training or test set files.
    							     	None : No verification.
    							     	Verified : The training or test set file is in the correct format.
    							     	Denied : The training or test set file is not in the correct format.
    							     */
    ...
    std::mutex* getMutex();//For GUI. Returns _mutex.
    ActionMode getActionMode()const;//For GUI. Returns _actionMode.
    int getCountSample()const;//For GUI. Returns _countSample.
    double getResultTest()const;//For GUI. Returns _resultTest.
    int getCountErrorOfTest()const;//For GUI. Returns _countErrorOfTest.
    int getCountAllOutputOfTest()const;//For GUI. Returns _countAllOutputOfTest.
    int getIndexSampleOfTest()const;//For GUI. Returns _indexSampleOfTest.
    size_t getIndexOutputOfTest()const;//For GUI. Returns _indexOutputOfTest.
    double getOutputOfTest()const;//For GUI. Returns _outputOfTest.
    double getExpectedOutputOfTest()const;//For GUI. Returns _expectedOutputOfTest.
    double getSingleOutputOfRun()const;//For GUI. Returns _singleOutputOfRun.
    size_t getIndexSingleOutputOfRun()const;//For GUI. Returns _indexSingleOutputOfRun.
    VerificationSamplesFile getVerificationSamplesFile()const;//For GUI. Returns _verificationSamplesFile.
    void resetVerificationSamplesFile();//For GUI. Assigns 'VerificationSamplesFile::None' value to _verificationSamplesFile.
    void setStopTraining(const bool& stop);//For GUI. Assigns the parameter value to _stopTraining.
    StatusTraining getStatusTraining()const;//For GUI. Returns _statusTraining.
    std::string getTrainingErrorMessage()const;//For GUI. Returns _trainingErrorMessage.
    
    const int*const* getImageAsArray(const int& countSample, const std::string& samplesFile);//For MNIST. Returns training or testing samples(images) as an array. Parameters :  Number of samples(images) of training or testing set(countSample) - Training or testing set file (samplesFile).
    ...
    bool train(const int& countSample, const std::string& trainingSamplesFile);// Trains the neural network in terminal. Parameters : Number of samples of the training set(countSample) - Training set file (trainingSamplesFile).
    double* run(const int*const inputsOriginal);// Calculates the outputs of the inputs passed to the network in terminal. Parameters : Input values (inputsOriginal).
    double test(const int& countSample,const std::string& testSamplesFile);// Tests the neural network in terminal. Parameters : Number of samples of the test set(countSample) - Test set file (testSamplesFile).

    void trainForGUI(const int& countSampleCorrected, const std::string& trainingSamplesFile);//For GUI. Trains the neural network in GUI. Parameters : Number of samples of the training set(countSampleCorrected) - Training set file (trainingSamplesFile).
    void runForGUI(const int*const inputsOriginal,std::promise<bool> &promiseFinish);//For GUI. Calculates the outputs of the inputs passed to the network in GUI. Parameters : Input values (inputsOriginal) - The std::promise object that signals to the GUI that this process is finished so that this function can be executed as multithreading(promiseFinish).
    void testForGUI(const int& countSampleCorrected,const std::string& testSamplesFile);//For GUI. Tests the neural network in GUI. Parameters : Number of samples of the test set(countSampleCorrected) - Test set file (testSamplesFile).
...
private:
    ...
    VerificationSamplesFile _verificationSamplesFile;//For GUI. Stores whether the training or test set files have been verified as a VerificationSamplesFile object.

    StatusTraining _statusTraining;//For GUI. Stores the current status of the training of the artificial neural network as a StatusTraining object.
    int _countSample;//For GUI. Number of samples in training or testing file.
    bool _stopTraining;//For GUI. Stores whether the training has been stopped or not. If stopped True, else False. 
    ActionMode _actionMode;//For GUI. Stores the action mode of the artificial neural network as a ActionMode object.
    std::mutex* _mutex;/*For GUI.
    			Training, testing and running processes execute as multithreading to prevent GUI objects from locking and freezing.
    			For each resource shared by different threads, a mutex is created to regulate access to the resource.
    			The code snippets where the shared resource is accessed is called the critical section.
    			This pointer object is the mutex we will use to ensure that critical sections in the training, testing and runnig processes can be used safely by different threads.
    		      */

    double _resultTest;//For GUI. Stores the result of the testing. Contains a value between 0 and 1. If the value is 1.0, success rate means 100%, and if it is 0.0, the success rate means 0%. 
    int _countErrorOfTest;//For GUI. Stores how many samples failed as a result of the testing.
    int _countAllOutputOfTest;//For GUI. Stores the result of multiplying the number of test samples by the number of real-world data which in the output layer. It is used to calculate the success rate of the testing.
    int _indexSampleOfTest;//For GUI. Stores the index of the sample that is in the test samples file and also currently passed to the artificial neural network.
    size_t _indexOutputOfTest;//For GUI. Stores the index of the calculated output value of the current test sample. Because can be more 1 the number of real-world data in the output layer.
    double _outputOfTest;//For GUI. Stores the calculated output value of the current test sample.
    double _expectedOutputOfTest;//For GUI. Stores the expected output value of the current test sample.

    double _singleOutputOfRun;//For GUI. Stores the result of the running for each the real-world data which in the output layer.
    size_t _indexSingleOutputOfRun;//For GUI. Stores the index of the value stored by variable _singleOutputOfRun.

    std::string _trainingErrorMessage;//For GUI. Stores the message of the error that caused the training to fail in order to inform the user.

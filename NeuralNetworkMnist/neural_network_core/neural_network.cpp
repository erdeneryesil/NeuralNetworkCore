...
#include<iomanip>
#include <csignal>
#include"../gui/form_main.h" //for GUI
...
NeuralNetwork::NeuralNetwork(const double& lambda,const double& alfa,const InputLayerPattern& inputLayerPattern,const HiddenLayerPattern& hiddenLayerPattern,const OutputLayerPattern& outputLayerPattern,const std::string& parametersFile,const char& separator,const double& randomWeightMin,const double& randomWeightMax){
    ...
    this->_stopTraining=false;
    this->_actionMode=ActionMode::Idle;
    this->_countSample=INT_0;
    this->_verificationSamplesFile=NeuralNetwork::VerificationSamplesFile::None;
    this->_trainingErrorMessage="";

    signal(SIGINT,SignalEpochHandler::handleSignal); //for GUI
...
std::mutex* NeuralNetwork::getMutex(){
    return this->_mutex;
}
NeuralNetwork::ActionMode NeuralNetwork::getActionMode()const{
    return this->_actionMode;
}
int NeuralNetwork::getCountSample()const{
    return this->_countSample;
}
double NeuralNetwork::getResultTest()const{
    return this->_resultTest;
}
int NeuralNetwork::getCountErrorOfTest()const{
    return this->_countErrorOfTest;
}
int NeuralNetwork::getCountAllOutputOfTest()const{
    return this->_countAllOutputOfTest;
}
int NeuralNetwork::getIndexSampleOfTest()const{
    return this->_indexSampleOfTest;
}
size_t NeuralNetwork::getIndexOutputOfTest()const{
    return this->_indexOutputOfTest;
}
double NeuralNetwork::getOutputOfTest()const{
    return this->_outputOfTest;
}
double NeuralNetwork::getExpectedOutputOfTest()const{
    return this->_expectedOutputOfTest;
}
double NeuralNetwork::getSingleOutputOfRun()const{
    return this->_singleOutputOfRun;
}
size_t NeuralNetwork::getIndexSingleOutputOfRun()const{
    return this->_indexSingleOutputOfRun;
}
std::string NeuralNetwork::getTrainingErrorMessage()const{
    return this->_trainingErrorMessage;
}
const int*const* NeuralNetwork::getImageAsArray(const int& countSample, const std::string& samplesFile){
    return this->_convertSamplesFileToArray(countSample,samplesFile);
}
...
void NeuralNetwork::setStopTraining(const bool& stop){
    this->_mutex->lock();
    this->_stopTraining=stop;
    this->_mutex->unlock();
}
NeuralNetwork::StatusTraining NeuralNetwork::getStatusTraining()const{
    return this->_statusTraining;
}
NeuralNetwork::VerificationSamplesFile NeuralNetwork::getVerificationSamplesFile()const{
    return this->_verificationSamplesFile;
}
void NeuralNetwork::resetVerificationSamplesFile(){
    this->_verificationSamplesFile=VerificationSamplesFile::None;
}
...
void NeuralNetwork::trainForGUI(const int& countSampleCorrected, const std::string& trainingSamplesFile){
    //int countSampleCorrected=this->_correctCountSampleInFile(countSampleCorrected,trainingSamplesFile);

    this->_mutex->lock();
    this->_statusTraining=StatusTraining::Ongoing;
    this->_stopTraining=false;
    this->_logEW->addNewTrainingInFile(countSampleCorrected);
    this->_actionMode=ActionMode::Train;
    this->_countSample=countSampleCorrected;
    this->_mutex->unlock();

    const int*const* trainingSamplesOriginal;

    try {
        trainingSamplesOriginal=this->_convertSamplesFileToArray(countSampleCorrected,trainingSamplesFile);
    }catch (std::exception& e) {
        this->_mutex->lock();
        this->_verificationSamplesFile=NeuralNetwork::VerificationSamplesFile::Denied;
        this->_actionMode=ActionMode::Idle;
        this->_mutex->unlock();
    ...
    this->_mutex->lock();
    this->_verificationSamplesFile=NeuralNetwork::VerificationSamplesFile::Verified;
    this->_mutex->unlock();
    ...
    do{
        ...
        for(int i=INT_0;i<countSampleCorrected;i++){
            ...
            this->_mutex->lock();
            if(this->_stopTraining){
                this->_statusTraining=StatusTraining::Halted;
                raise(SIGINT);
                this->_stopTraining=false;
                this->_actionMode=ActionMode::Idle;
                this->_countSample=INT_0;
        ...
        if(!this->_controlError(totalErrorInAnEpoch)){
            this->_deleteTrainingArrays(countSampleCorrected,trainingSamplesOriginal,trainingSamplesInputsTransformed,trainingSamplesOutputsTransformed);
            this->_mutex->lock();
            this->_statusTraining=StatusTraining::Failed;
            this->_trainingErrorMessage=TRAINING_ERROR_MESSAGE_INCORRECT_ERROR_VALUE;
            raise(SIGINT);
            this->_stopTraining=false;
            this->_actionMode=ActionMode::Idle;
            this->_countSample=INT_0;
    ...
    if(!this->_controlOutputs(countSampleCorrected,trainingSamplesOriginal)){
        ...
        this->_mutex->lock();
        this->_statusTraining=StatusTraining::Failed;
        this->_trainingErrorMessage=TRAINING_ERROR_MESSAGE_INCORRECT_OUTPUT_VALUES;
        raise(SIGINT);
        this->_stopTraining=false;
        this->_actionMode=ActionMode::Idle;
        this->_countSample=INT_0;
    ...
    if(!this->_controlWeights()){
        ...
        this->_mutex->lock();
        this->_statusTraining=StatusTraining::Failed;
        this->_trainingErrorMessage=TRAINING_ERROR_MESSAGE_INCORRECT_WEIGHT_VALUES;
        raise(SIGINT);
        this->_stopTraining=false;
        this->_actionMode=ActionMode::Idle;
        this->_countSample=INT_0;
    ...
    this->_deleteTrainingArrays(countSampleCorrected,trainingSamplesOriginal,trainingSamplesInputsTransformed,trainingSamplesOutputsTransformed);
    this->_mutex->lock();
    this->_statusTraining=StatusTraining::Succesfull;
    raise(SIGINT);
    this->_stopTraining=false;
    this->_actionMode=ActionMode::Idle;
    this->_countSample=INT_0;
...
void NeuralNetwork::runForGUI(const int*const inputsOriginal, std::promise<bool> &promiseFinish){
    this->_mutex->lock();
    this->_actionMode=ActionMode::Run;
    this->_mutex->unlock();
    ...
    for(size_t i=INT_0;i<this->_outputLayerPattern->getCountOriginal();i++){
        ...
        case Data::Type::Binary:
        {
            Binary data(this->_outputLayerPattern->getLength(i),outputDigits,this->_outputLayerPattern->getLimit(i)->getOriginalMax(),this->_outputLayerPattern->getLimit(i)->getScaledMin(),this->_outputLayerPattern->getLimit(i)->getScaledMax());
            this->_mutex->lock();
            this->_singleOutputOfRun=data.getOriginal();
            this->_mutex->unlock();
            break;
        }
        case Data::Type::Flag:
        ...
        case Data::Type::Narrowed:
        {
            ...
            Narrowed data(outputDigits[INT_0],limit->getScaledMin(),limit->getScaledMax());
            this->_mutex->lock();
            this->_singleOutputOfRun=data.getOriginal(limit->getOriginalMin(),limit->getOriginalMax());
            this->_mutex->unlock();
            break;
        }
        case Data::Type::Native:
        ...
        this->_mutex->lock();
        this->_indexSingleOutputOfRun=i;
        this->_mutex->unlock();
...
void NeuralNetwork::testForGUI(const int& countSampleCorrected, const std::string& testSamplesFile){
    //int countSampleCorrected=this->_correctCountSampleInFile(countSampleCorrected,testSamplesFile);

    this->_mutex->lock();
    this->_actionMode=ActionMode::Test;
    this->_resultTest=DOUBLE_NEG_1;
    this->_countSample=countSampleCorrected;
    this->_mutex->unlock();

    const int*const* testSamplesOriginal;

    try {
        testSamplesOriginal=this->_convertSamplesFileToArray(countSampleCorrected,testSamplesFile);
    }  catch (std::exception& e) {
        this->_mutex->lock();
        this->_verificationSamplesFile=NeuralNetwork::VerificationSamplesFile::Denied;
        this->_actionMode=ActionMode::Idle;
    ...
    this->_countAllOutputOfTest=countSampleCorrected*this->_outputLayerPattern->getCountOriginal();
    this->_mutex->unlock();
    ...
    for(int i=INT_0;i<countSampleCorrected;i++){
        ...
        for(size_t j=INT_0;j<this->_outputLayerPattern->getCountOriginal();j++){
            ...
            if(std::round(outputsOriginal[j])!=expectedOutputsOriginal[j]){
                ...
                this->_indexSampleOfTest=i;
                this->_indexOutputOfTest=j;
                this->_outputOfTest=outputsOriginal[j];
                this->_expectedOutputOfTest=expectedOutputsOriginal[j];
                this->_mutex->unlock();
                raise(SIGINT);
            }
        }
    ...
    this->_mutex->lock();
    this->_resultTest=DOUBLE_POS_1-(double)this->_countErrorOfTest/(double)this->_countAllOutputOfTest;
    this->_mutex->unlock();

...
FormMain::FormMain(){
    ...
    this->_spacerTop=new QSpacerItem(0,0,QSizePolicy::Minimum,QSizePolicy::Expanding);
    this->_spacerBottom=new QSpacerItem(0,0,QSizePolicy::Minimum,QSizePolicy::Expanding);
    ...
    this->_layoutNNAsset->addItem(this->_spacerTop);
    ...
    this->_textEditInformation->setFixedSize(FORM_MAIN_CONTROL_WIDTH,FORM_MAIN_LIST_HEIGHT);
    this->_layoutNNAction->addItem(this->_spacerBottom);
...
FormMain::~FormMain(){
    ...
    this->_layoutNNAction->removeItem(this->_spacerBottom);
    delete this->_spacerBottom;
    ...
    this->_layoutNNAsset->removeItem(this->_spacerTop);
    delete _spacerTop;
...
void FormMain::slotSamplesFileParameters(int countSample, std::string samplesFile){

    QString textLabelInformation=this->_labelInformation->text();
    QString textPushButtonTrain=this->_pushButtonTrain->text();

    int countLinesOfFile=FormMain::NeuralNetworkObject()->getCountLinesOfFile(samplesFile);
    int countSampleCorrected=countSample;
    if(countLinesOfFile<countSample){
        countSampleCorrected=countLinesOfFile;
        QMessageBox::information(this,"Correction","Corrected the number of samples. ",QMessageBox::Ok);
    }
...
void FormMain::slotRunParameters(int * const inputs){
    ...
    for(size_t i=0;i<FormMain::NeuralNetworkObject()->getInputLayerPattern()->getCountOriginal();i++){
        this->_runInfo.append("Input"+QString::number(i)+"\t: ");
        this->_runInfo.append(QString::number(inputs[i]));
        this->_runInfo.append("\n");
    }
    this->_runInfo.append("---------------------");
...
void FormMain::slotAddLineForNNDiagram(qreal x1,qreal y1,qreal x2, qreal y2,const QPen&pen){
    this->_graphicSceneNN->addLine(x1,y1,x2,y2,pen);
}
void FormMain::slotAddEllipseForNNDiagram(qreal x,qreal y,qreal w, qreal h,const QPen&pen,const QBrush&brush){
...
void FormMain::showRunInfo(const size_t& indexSingleOutput,const double& singleOutput){
    this->_runInfo.append("\nOutput"+QString::number(indexSingleOutput)+"\t: "+QString::number(std::round(singleOutput)));

    Data::Type outputType=FormMain::NeuralNetworkObject()->getOutputLayerPattern()->getType(indexSingleOutput);
    if(outputType==Data::Type::Narrowed || outputType==Data::Type::Native)
        this->_runInfo.append("\t( "+QString::number(singleOutput)+" )");
